...
export default function StoreEmailLog() {
  ...
  return (
    <div className="flex flex-col gap-6 relative pb-10">
      <div>
        <h1 className="text-[1.25rem] font-bold text-[var(--text-primary)]">{t('storeEmailLog.pageTitle', 'Email Log')}</h1>
        <p className="mt-[3px] text-[0.8125rem] text-[var(--text-muted)]">{t('storeEmailLog.pageSubtitle', 'Review email communications and supplier responses regarding purchase orders.')}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-2">
        <div className="flex-1 max-w-sm relative">
          ...
          <input 
            ...
            className="w-full bg-[var(--bg-input)] border border-[var(--border)] text-[var(--text-primary)] pl-9 pr-3 py-2 rounded-lg text-[0.8125rem] outline-none focus:border-[#8B5CF6] transition-colors"
          />
        </div>
        <select 
          value={poFilter} 
          onChange={e => setPoFilter(e.target.value)} 
          className="bg-[var(--bg-input)] border border-[var(--border)] text-[var(--text-secondary)] px-3 py-2 rounded-lg text-[0.8125rem] outline-none focus:border-[#8B5CF6] transition-colors"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredLogs.length === 0 ? (
          <div className="col-span-full text-center py-10 text-[var(--text-muted)]">{t('storeEmailLog.noCommunications', 'No communications found.')}</div>
        ) : (
          filteredLogs.map(log => (
            <Panel key={log.id} noPadding className="flex flex-col shadow-sm">
              <div className="p-3.5 px-4 bg-[var(--bg-card)] border-b border-[var(--border)] flex items-center gap-2.5">
                <StatusDot status={log.status} />
                <span className="text-[0.85rem] font-semibold text-[var(--text-primary)] truncate flex-1">{log.supplier}</span>
              </div>
              <div className="p-4 flex-1 flex flex-col gap-2">
                ...
                <h3 className="text-sm font-bold text-[var(--text-primary)] leading-snug">{log.subject}</h3>
                <p className="text-xs text-[var(--text-secondary)] line-clamp-2 mt-1">{log.thread[log.thread.length - 1].body}</p>
              </div>
              <div className="p-3 px-4 bg-[var(--bg-card)] border-t border-[var(--border)] flex justify-between items-center">
                <button 
                ...
                <button 
                  ...
                  className="w-[26px] h-[26px] rounded flex items-center justify-center text-[var(--text-muted)] hover:text-[#F87171] hover:bg-[rgba(239,68,68,0.1)] transition-colors"
                  title="Delete Log"
                >
      ...
      >
        <div className="mb-2">
          <h4 className="font-bold text-lg text-[var(--text-primary)] mb-1 leading-snug">{selectedLog?.subject}</h4>
          <p className="text-sm text-[#D8B4FE]">{t('storeEmailLog.with', 'With:')} {selectedLog?.supplier}</p>
        </div>
        <div className="flex flex-col gap-2 mt-2">
          {selectedLog?.thread.map((msg, i) => (
            <div key={i} className="bg-[var(--bg-input)] border border-[var(--border)] rounded-lg p-4 relative">
              <div className="flex items-center gap-2 mb-2">
                <span className="font-semibold text-[var(--text-primary)] text-sm">{msg.from}</span>
                <span className="w-1 h-1 rounded-full bg-[var(--text-muted)]" />
                <span className="text-xs text-[var(--text-muted)]">{msg.date}</span>
              </div>
              <div className="text-sm text-[var(--text-secondary)] whitespace-pre-wrap leading-relaxed">{msg.body}</div>
            </div>
          ))}
        </div>
      </Modal>
