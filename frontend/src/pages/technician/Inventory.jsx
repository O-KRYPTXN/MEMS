...
const RequestStatusBadge = ({ status }) => {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[0.65rem] font-bold ${map[status] || ''}`}>{status}</span>
}
...
export default function TechnicianInventory() {
  ...
  return (
    <div className="flex flex-col gap-6 relative pb-10">
      <div>
        <h1 className="text-[1.25rem] font-bold text-[#E2E8F0]">Spare Parts Lookup</h1>
        <p className="mt-[3px] text-[0.8125rem] text-[#5A6A85]">Check central store inventory and request parts for your active work orders.</p>
      </div>

      <div className="bg-[#181D2A] border border-[#1F2A40] rounded-xl mb-2 overflow-hidden">
        <div className="p-4 border-b border-[#1F2A40]">
          <h2 className="text-sm font-bold text-[#E2E8F0]">My Recent Requests</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-[#1A2235] border-b border-[#1F2A40]">
                {['Request ID', 'Part Name', 'Qty', 'Date', 'Status'].map(h => (
      ...
      <div className="bg-[#181D2A] border border-[#1F2A40] rounded-b-xl rounded-tr-xl overflow-hidden mt-[-10px]">
        <div className="bg-[#131720] border-b border-[#1F2A40] p-3 px-4 flex flex-wrap gap-2.5 items-center">
          <div className="flex items-center gap-2 flex-1 min-w-[200px] h-[34px] bg-[#0F1117] border border-[#1F2A40] rounded-lg px-3 focus-within:border-[#F59E0B] transition-colors">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search part name or ID..." className="flex-1 min-w-0 bg-transparent border-none outline-none text-[#E2E8F0] text-[0.8125rem]" />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-[#1A2235] border-b border-[#1F2A40]">
                {['Part ID', 'Part Name', 'Category', 'Stock Level', 'Status', 'Actions'].map(h => (
                  <th key={h} className="p-4 text-[0.75rem] font-bold text-[#5A6A85] uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-[#1F2A40]">
              {filteredParts.length === 0 ? <tr><td colSpan={6} className="p-8 text-center text-[#5A6A85]">No parts found.</td></tr> : filteredParts.map(p => (
                <tr key={p.id} className="hover:bg-[rgba(255,255,255,0.02)]">
                  ...
                  <td className="p-4">
                    <button onClick={() => { setSelectedPart(p); setShowReqModal(true) }} className="px-2.5 py-1 text-[11px] font-bold bg-transparent border border-[#1F2A40] text-[#94A3B8] rounded-md hover:bg-[#1A2235] hover:text-[#E2E8F0] transition-colors">Request Part</button>
                  </td>
                </tr>
              ))}
            </tbody>
