...
export default function Reports() {
  ...
  const columns = useMemo(() => [
    { key: 'id', label: t('reports.reportId'), render: val => <span className="font-mono text-[#3B82F6] font-semibold text-xs">{val}</span> },
    { key: 'name', label: t('reports.reportName'), render: val => <div className="truncate max-w-[260px] font-medium text-[var(--text-primary)]">{val}</div> },
    { key: 'category', label: t('reports.category'), render: val => <CategoryBadge category={val} /> },
    { key: 'format', label: t('reports.format'), render: val => <FormatBadge format={val} /> },
    { key: 'date', label: t('reports.date'), render: val => <span className="text-[#94A3B8]">{formatDate(val)}</span> },
    { key: 'by', label: t('reports.generatedBy') },
    { key: 'size', label: t('reports.size'), render: val => <span className="text-[var(--text-muted)] text-xs font-mono">{val}</span> },
    { key: 'id', label: t('reports.actions'), align: 'right',
      render: (val, row) => (
        <div className="flex justify-end gap-1.5">
          <button onClick={() => { setSelectedReport(row); setShowViewModal(true) }} className="w-7 h-7 rounded-md bg-[var(--bg-hover)] border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)]" title={t('reports.view')}>
          ...
          <button onClick={() => showToast(`Downloading ${row.name}...`, TOAST_COLORS.admin)} className="w-7 h-7 rounded-md bg-[var(--bg-hover)] border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)]" title={t('reports.download')}>
  ...
  const renderPagination = () => (
    <div className="flex items-center justify-between px-5 py-3 border-t border-[var(--border)]">
      <span className="text-[0.8rem] text-[var(--text-muted)]">
        {filteredReports.length === 0 ? t('reports.noReports') : t('reports.showingResults', { start, end, total: filteredReports.length })}
      </span>
  ...
  return (
    <div className="flex flex-col gap-6 relative pb-10">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-[1.25rem] font-bold text-[var(--text-primary)]">{t('reports.pageTitle')}</h1>
          <p className="mt-[3px] text-[0.8125rem] text-[var(--text-muted)]">{t('reports.pageSubtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-[16px]">
        ...
        <KPICard title={t('reports.woCompletionRate')} value="87%" iconVariant="green" iconPath="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        <KPICard title={t('reports.totalDevices')} value="142" iconVariant="orange" iconPath="M21 7.5l-2.25-1.313M21 7.5v2.25m0-2.25l-2.25 1.313M3 7.5l2.25-1.313M3 7.5l2.25 1.313M3 7.5v2.25m9 3l2.25-1.313M12 12.75l-2.25-1.313M12 12.75V15m0 6.75l2.25-1.313M12 21.75V19.5m0 2.25l-2.25-1.313m0-16.875L12 2.25l2.25 1.313M21 14.25v2.25l-2.25 1.313m-13.5 0L3 16.5v-2.25" />
        <KPICard title={t('reports.criticalAlerts')} value="7" danger iconVariant="red" iconPath="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <Panel padding="p-5" className="lg:col-span-2 flex flex-col">
          <div className="text-[0.875rem] font-bold text-[var(--text-primary)] mb-4">{t('reports.pmComplianceTrend')}</div>
        ...
        <Panel padding="p-5" className="lg:col-span-1 flex flex-col items-center">
          <div className="text-[0.875rem] font-bold text-[var(--text-primary)] mb-4 w-full text-left">{t('reports.monthlyFaultSummary')}</div>
          <div className="relative w-full flex-1 flex items-center justify-center min-h-[240px]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-2xl font-bold text-[var(--text-primary)]">181</span>
        ...
        <Panel padding="p-5" className="lg:col-span-2 flex flex-col">
          <div className="text-[0.875rem] font-bold text-[var(--text-primary)] mb-4">{t('reports.maintenanceCost')}</div>
          <div className="flex-1 min-h-[240px]">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <Panel padding="p-5" className="lg:col-span-1 flex flex-col">
          <div className="text-[0.875rem] font-bold text-[var(--text-primary)] mb-5">{t('reports.sparePartsConsumption')}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_280px] gap-5 mt-2">
        <Panel noPadding className="flex flex-col">
          <div className="p-4 border-b border-[var(--border)] flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 w-[220px] h-[34px] px-3 bg-[var(--bg-input)] border border-[var(--border)] rounded-lg">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('reports.searchPlaceholder')}
            ...
            <select value={formatFilter} onChange={e => setFormatFilter(e.target.value)} className={selectCls + " h-[34px]"}>
              <option value="">{t('reports.allFormats')}</option>
          ...
          <div className="flex-1">
            <DataTable columns={columns} data={paginated} emptyMessage={t('reports.noReports')} />
          </div>
          {renderPagination()}
        </Panel>
        ...
        <Panel noPadding className="flex flex-col">
          <div className="p-4 border-b border-[var(--border)]">
            <div className="text-[0.875rem] font-bold text-[var(--text-primary)]">{t('reports.generateReport')}</div>
          </div>
            ...
            {quickReports.map((r, i) => (
              <div key={i} onClick={() => handleQuickGenerate(r)} className="group flex items-center gap-3 p-2.5 rounded-lg hover:bg-[rgba(255,255,255,0.03)] cursor-pointer transition-colors">
                ...
                <div className="shrink-0 flex items-center gap-2">
                  ...
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3 h-3 text-[var(--text-muted)] group-hover:text-[var(--text-primary)] transition-colors"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
                </div>
              </div>
            ))}
          </div>
      ...
      <Modal
        ...
        title={
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[15px] font-bold text-[var(--text-primary)] tracking-normal leading-tight">{t('reports.reportViewer')}</div>
              <div className="text-[12px] text-[var(--text-muted)] font-mono leading-tight mt-0.5">{selectedReport?.id} • {selectedReport?.name}</div>
            </div>
          </div>
        }
        maxWidth="42rem"
        footer={
          <>
            ...
            <button type="button" onClick={() => { showToast(`Downloading ${selectedReport?.id}...`, TOAST_COLORS.admin); setShowViewModal(false); }} className="px-[20px] py-[10px] bg-[#3B72F6] hover:bg-[#2558D8] rounded-lg text-white text-[13px] font-medium transition-colors flex items-center gap-[6px]">
              ...
              {t('reports.download')} {selectedReport?.format}
            </button>
          </>
        }
      >
        <div className="p-5 bg-[var(--bg-input)] rounded-xl h-[320px] overflow-y-auto text-[13px] text-[var(--text-primary)] font-mono leading-relaxed mt-1 border border-[var(--border)]">
          <div className="mb-4 text-[#3B82F6] font-bold">► {t('reports.executiveSummary')}</div>
          <p className="mb-6 text-[var(--text-muted)]">Generated on {selectedReport && formatDate(selectedReport.date)} by {selectedReport?.by}. This document contains system-generated analytics for the category: {selectedReport?.category.toUpperCase()}.</p>
          
          <div className="mb-4 text-[#3B82F6] font-bold">► {t('reports.reportDetails')}</div>
          <div className="mb-6 space-y-2 text-[var(--text-muted)]">
            ...
            <br/>
            <div className="border-l-2 border-[var(--border)] pl-3 text-[var(--text-primary)]">
          ...
          <div className="text-center mt-10 text-[var(--text-muted)] font-bold">{t('reports.endOfReport')}</div>
        </div>
      </Modal>
    </div>
  )
