...
function getAvatarColor(role) {
  const map = { 'Administrator': '#3B72F6', 'Supervisor': '#10B981', 'Technician': '#F59E0B', 'Storekeeper': '#8B5CF6' };
  return map[role] || '#3B72F6';
}

const ROLE_BADGES = {
  Administrator: 'bg-[rgba(239,68,68,0.15)] text-[#F87171] border border-[rgba(239,68,68,0.3)]',
...
const RoleBadge = ({ role }) => {
  ...
  return <span className={`inline-block px-2 py-0.5 rounded-[6px] text-[11px] font-semibold ${cls}`}>{role}</span>
}
...
const TABS = [
  ...
  { label: 'Storekeepers', value: 'Storekeeper' },
  { label: 'Pending Requests', value: 'pending' },
]

const ROLE_OPTS = [
  ['', 'All'], ['Administrator', 'Administrator'], ['Supervisor', 'Supervisor'],
...
export default function Users() {
  ...
  const userColumns = useMemo(() => [
    ...
    { key: 'lastActive', label: 'Last Active', render: val => val !== 'Never' ? formatDate(val) : val },
  ...
  const pendingColumns = useMemo(() => [
    ...
    { key: 'email', label: 'Email' },
    { key: 'role', label: 'Requested Role', render: val => <RoleBadge role={val} /> },
    { key: 'department', label: 'Department' },
    { key: 'submitted', label: 'Submitted', render: val => formatDate(val) },
    { key: 'status', label: 'Status', render: val => <PendingStatusBadge status={val} /> },
    { key: 'id', label: 'Actions', align: 'right',
      render: (val, row) => {
  ...
  const renderPagination = () => (
    <div className="flex items-center justify-between px-5 py-3 border-t border-[#1F2A40]">
      <span className="text-[0.8rem] text-[#5A6A85]">
        Showing {start}–{end} of {currentList.length} items
      </span>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-[1.25rem] font-bold text-[#E2E8F0]">User Management</h1>
        <p className="mt-[3px] text-[0.8125rem] text-[#5A6A85]">Manage hospital staff accounts and Role-Based Access Control (RBAC) permissions.</p>
      </div>

      <div className="grid grid-cols-2 xl:grid-cols-4 gap-[16px]">
        <KPICard title="Total System Users" value={totalUsers} iconVariant="blue" iconPath="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
        <KPICard title="Technicians" value={techCount} iconVariant="green" iconPath="M11.42 15.17L17.25 9.34m-5.83 5.83l-2.83 2.83a2.828 2.828 0 11-4-4l2.83-2.83m5.83 5.83l5.66-5.66a2.828 2.828 0 00-4-4l-5.66 5.66m5.83 5.83a2.828 2.828 0 01-4 4l-5.66-5.66a2.828 2.828 0 014-4l5.66 5.66z" />
        <KPICard title="Supervisors" value={superCount} iconVariant="orange" iconPath="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
        <KPICard title="Administrators" value={adminCount} danger iconVariant="red" iconPath="M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 013 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 011.043-3.296 3.746 3.746 0 013.296-1.043A3.746 3.746 0 0112 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 013.296 1.043 3.746 3.746 0 011.043 3.296A3.745 3.745 0 0121 12z" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-[12px]">
        <div className="flex items-center gap-2 w-[240px] h-[36px] px-3 bg-[#1A2235] border border-[#1F2A40] rounded-lg">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search users by name, email..."
        ...
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className={selectCls}>
          {ROLE_OPTS.map(([v, l]) => <option key={v||'all'} value={v}>{v ? `Role: ${l}` : 'Role: All'}</option>)}
        </select>
        <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)} className={selectCls}>
          {DEPT_OPTS.map(([v, l]) => <option key={v||'all'} value={v}>{v ? `Department: ${l}` : 'Department: All'}</option>)}
        </select>
      ...
      <div className="bg-[#181D2A] border border-[#1F2A40] rounded-[12px] overflow-hidden">
        {activeTab === 'pending' ? (
          <DataTable columns={pendingColumns} data={paginated} emptyMessage="No pending requests match your filters." />
        ) : (
          <DataTable columns={userColumns} data={paginated} emptyMessage="No users match your filters." />
        )}
        {renderPagination()}
      </div>
