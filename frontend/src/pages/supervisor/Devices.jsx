...
const DeviceStatusBadge = ({ status }) => {
  ...
  const labelMap = {
    'Operational': t('supDevices.operational'),
    'Faulty': t('supDevices.faulty'),
    'Under Maintenance': t('supDevices.underMaintenance'),
    'Repaired': t('supDevices.repaired'),
  }
  ...
  return (
    <div className="flex flex-col gap-6 relative pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: t('supDevices.totalItems'), value: kpis.total, icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 3.75H6.912a2.25 2.25 0 00-2.15 1.588L2.35 13.177a2.25 2.25 0 00-.1.661V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18v-4.162c0-.224-.034-.447-.1-.661L19.24 5.338a2.25 2.25 0 00-2.15-1.588H15M2.25 13.5h3.86a2.25 2.25 0 012.012 1.244l.256.512a2.25 2.25 0 002.013 1.244h3.218a2.25 2.25 0 002.013-1.244l.256-.512a2.25 2.25 0 012.013-1.244h3.859M12 3v8.25m0 0l-3-3m3 3l3-3" />, colorClass: 'bg-[rgba(20,184,166,0.12)] text-[#14B8A6]' },
          { label: t('supDevices.operational'), value: kpis.operational, icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />, colorClass: 'bg-[rgba(74,222,128,0.12)] text-[#4ADE80]' },
      ...
      >
        <form id="fault-form" onSubmit={handleReportFault} className="flex flex-col gap-4 mt-1">
          ...
          <SelectField label={t('supDevices.selectDevice')} value={faultDeviceId} onChange={e => setFaultDeviceId(e.target.value)} placeholder={t('supDevices.selectOperationalDevice')} options={devices.filter(d => d.status === 'Operational').map(d => ({value: d.id, label: `${d.name} (${d.id})`}))} />
          ...
          <SelectField label={t('common.priority')} defaultValue="High" options={['High', 'Medium', 'Low']} />
        </form>
      </Modal>
